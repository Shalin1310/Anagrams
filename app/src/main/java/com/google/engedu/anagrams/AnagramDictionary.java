...
public class AnagramDictionary {
    ...
    /*public void showlist(HashMap<Integer,ArrayList<String>> abc){
    ...
    public String pickGoodStarterWord() {
        ...
        ArrayList<String> item=sizetoword.get(wordlength);
        if(item.size()==wordlength&&wordlength<=MAX_WORD_LENGTH){
            List<String> items=new ArrayList<>();
            for (int i=wordlength;i<wordList.size();i++){
                items=getAnagramsWithOneMoreLetter(wordList.get(i));
                if(items.size()>=MIN_NUM_ANAGRAMS)
                {
                    wordlength++;
        ...
        do{
            StarterWords = sizetoword.get(wordlength);
